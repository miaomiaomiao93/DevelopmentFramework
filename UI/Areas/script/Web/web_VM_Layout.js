    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i = 0; i < data.Data.List.length; i++) {
                stringBuilder += '<div onclick="jumpDownLoad(\'' + data.Data.List[i].Id + '\')"><img style="width:77px;height:78px;" src="../../../' + data.Data.List[i].ThumbUrl + '" /><p class="introduce"><span>' + data.Data.List[i].GoodsName + '</span><br />';
                var count = data.Data.List[i].TagModels.length > 3 ? 3 : data.Data.List[i].TagModels.length;
                for (var j = 0; j < count ; j++) {
                    stringBuilder += '<span class="tag">' + data.Data.List[i].TagModels[j].TagName + '</span>';
                }
                stringBuilder += '<br /><span>' + data.Data.List[i].Company + '</span></p></div>';
            }
